...
library(logging)
library(pROC) # AUC
source("SeleccionMoPImputsBalanced.R")
source("fitTAN.R")
source("mutualInformation.R")
load("../Datasets/MIMOPB40.RData")
dag <- "TAN"
...
bnCV = foreach(i = 1:10,.packages = c("logging", "MoTBFs","bnlearn"))%dopar%{
  selective_tan(paste0(Nombre.Red,"_",i),target,balancedFold[[i]],nameLoss,
                      order = names(orden[[i]]),
                      fit.args=fit.args,verbose = T,
                      MI = MIMopB[[i]][[1]],root = names(orden[[i]])[1],parallel = F)
}
stopCluster(cl)
