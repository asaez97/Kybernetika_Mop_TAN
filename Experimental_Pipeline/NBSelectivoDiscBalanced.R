...
bnCV = foreach(i = 1:10,.packages = c("bnlearn", "MoTBFs","logging"))%dopar%{
  selective_nb(paste0(Nombre.Red,"_",i),target,balancedFold[[i]],nameLoss,
               order = names(orden[[i]]),
...
for(l in 1:k){
  ...
  listaAlmacenar$Variables <- paste0(names(bnCV[[l]]$fitted),collapse = ";")
  ...
  listaAlmacenar$nCont = sum(names(bnCV[[l]]$fitted)%in%varCont[[1]])
