...
source("fitTANGauss.R")
load("../Datasets/MIGaussB40.RData")
dag <- "TAN"
...
bnCV = foreach(i = 1:10,.packages = c("bnlearn","logging"))%dopar%{
  selective_tan_g(paste0(Nombre.Red,"_",i),target,balancedFold[[i]],nameLoss,
                  order = names(orden[[i]]),verbose = F,MI = MIGaussB[[i]],
...
for(l in 1:k){
  ...
  listaAlmacenar$nCont[l] = sum(varsModelo%in%varCont[[1]])
